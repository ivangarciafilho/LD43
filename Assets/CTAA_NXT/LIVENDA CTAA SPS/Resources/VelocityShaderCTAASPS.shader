//CTAA SPS VelocityShaderCTAASPS SHADER NXT V2
...
Shader "Hidden/VelocityShaderCTAASPS"
{
	...
	float DFFGF56uiuyyFRrfgFFR(float2 uv)
	{
	...
	float4 blit_frag_prepass(blit_v2f IN) : SV_Target
	{
		...
		float vs_dist = DFFGF56uiuyyFRrfgFFR(IN.ss_txc);
	...
	float4 frag(v2f IN) : SV_Target
	{
		...
		float scene_d = DFFGF56uiuyyFRrfgFFR(ss_txc);
