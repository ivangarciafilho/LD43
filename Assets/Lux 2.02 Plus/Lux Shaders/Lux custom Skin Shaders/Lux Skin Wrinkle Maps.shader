Shader "Lux/Human/Skin Wrinkle Maps" {
	Properties {
	...
	SubShader {
		Tags { "RenderType"="Opaque" "Special"="Wrinkles"}
		LOD 200
	...
	//	Wrinkle Maps
		...
		//	Wrinkle Maps
			#ifdef WRINKLE_MAPS
				float2 wrinkleUVs = IN.screenPos.xy / IN.screenPos.w;
				float3 normalOcclusion = tex2D(_NormalAndOcclusion, wrinkleUVs).rgb;
		...
		//	Wrinkle Maps
			#ifdef WRINKLE_MAPS
			/*
				fixed3 blurredWorldNormalSample = tex2Dbias(_NormalAndOcclusion, float4(wrinkleUVs, 0, _BumpBias));
				fixed3 blurredWorldNormal;
				blurredWorldNormal.xy = blurredWorldNormalSample.gb * 2.f - 1.f;
				blurredWorldNormal.z = sqrt(saturate(1.f - dot(blurredWorldNormal.xy, blurredWorldNormal.xy))); */
			//	As mips currently are not supported...
				fixed3 blurredWorldNormal = o.Normal;
			#else
