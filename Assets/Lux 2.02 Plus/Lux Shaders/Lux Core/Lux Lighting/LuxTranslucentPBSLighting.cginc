...
// keyword is needed by e.g. dynamic weather but might not be set in case we have a custom surface shader
...
{
	// s.Normal = normalize(s.Normal); // has been moved to GI
	#if defined(UNITY_PASS_FORWARDADD)
	//	Factor in Snow normal - only needed by Surface shaders
		s.Normal = normalize(lerp(s.Normal, s.SnowWorldNormal.xyz, s.SnowWorldNormal.w));
	#endif
...
inline half4 LightingLuxTranslucentSpecular_Deferred (SurfaceOutputLuxTranslucentSpecular s, half3 viewDir, UnityGI gi, out half4 outDiffuseOcclusion, out half4 outSpecSmoothness, out half4 outNormal)
{
	// energy conservation
	half oneMinusReflectivity;
	s.Albedo = EnergyConservationBetweenDiffuseAndSpecular (s.Albedo, s.Specular, /*out*/ oneMinusReflectivity);
...
//	Horizon Occlusion – deferred reflections
	...
	outSpecSmoothness = half4(s.Specular.r, s.ScatteringPower * 0.125, s.Translucency, s.Smoothness);
