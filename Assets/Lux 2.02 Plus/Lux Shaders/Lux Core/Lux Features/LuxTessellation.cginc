...
float4 LuxTessEdge (appdata v0, appdata v1, appdata v2)
{
    ...
    float4 tess;
    tess.xyz = clamp(1.0 - (dist - _MinDist) / (_MaxDist - _MinDist), 0.01, 1.0) * _Tess;
    // length of the edges
...
//  tess.xyz *= f;
	float3 f = (len * _ScreenParams.y / (_EdgeLength * dist));
//	Clamp tess to max edge length
	tess.xyz = min(tess.xyz, f);
