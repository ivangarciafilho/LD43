	...
	)
{
	...
	#if defined (_SNOW)
	//	In case we use global snow mapping the tangent normal does not contain the snownormal, so we simply lerp towards a flat one
		tangentNormal = (_SnowMapping == 1) ? lerp(tangentNormal, half3(0,0,1), saturate(lux.normalBlendFactor)) : tangentNormal;
	#endif
